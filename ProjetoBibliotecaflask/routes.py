...
def listar_autores():
    search = request.args.get('search')
    if search:
        autores = Autor.query.filter(Autor.nome.ilike(f'%{search}%')).all()
    else:
        autores = Autor.query.all()
...
def excluir_autor(autor_id):
    ...
    # Atualizar autor_id dos livros para NULL
    Livro.query.filter_by(autor_id=autor_id).update({'autor_id': None})
    db.session.commit()
